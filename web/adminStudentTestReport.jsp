            ...
            <nav id="sidebar" class="bg-dark fixed-left">
                <div class="sidebar-header">
                    <center>  <h4 class="domain">
                        Tests Taken by <%=username%></h4></center>
                    <br>
...
<%
    ...
    try {
        ...
        testid = "";
        while (rs.next()) {
            PreparedStatement pst = conn.prepareStatement("Select * from tests where TestID=?");
            pst.setInt(1,rs.getInt("TestID"));
            rst = pst.executeQuery();
            while(rst.next()){
                %>
                        <li class="active content1">
                            <p class="domain"><a href="adminStudentTestReport.jsp?UserName=<%=username%>&amp;UserId=<%=uid%>&amp;TestID=<%=rs.getInt("TestID")%>"><%=rst.getString("TestName")%></a></p>
                        </li>
                        <%
            }
        }
        ...
        hr.onreadystatechange = function(){
            if(hr.readyState==4 && hr.status==200){
                ...
                for(i=0;i< dataArray.length-1;i++){
                    ...
                    var test_link="adminStudentTestReport.jsp?UserName=<%=username%>&UserId=<%=uid%>&TestID=<%=testid%>";
